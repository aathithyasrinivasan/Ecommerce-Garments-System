...
// <i className='bi bi-pencil-square text-danger pb-2 fs-2'></i>
    ...
    const setOrder=()=>{
       ...
       products && products.map(product=>(
            data.rows.push({
                ...
                actions:(<Link to={`/admin/product/${product._id}`}><p className='pro-dete'><i className='bi bi-pencil-square text-danger pb-2 fs-2'></i></p></Link>)
              })
            ))
        return data
    }
